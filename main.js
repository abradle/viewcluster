var clusters = null;
function gotoMajorCluster(clusterInd){

var view_set = clusters[clusterInd].value;
setView(view_set);
...
clusters = null;
#clusters = clusterfck.hcluster(viewList, clusterfck.EUCLIDEAN_DISTANCE,
#        clusterfck.AVERAGE_LINKAGE, threshold);
var kmeans = new clusterfck.Kmeans();

// Calculate clusters.
var clusters = kmeans.cluster(viewList, 3);

// Calculate cluster index for a new data point.
var clusterIndex = kmeans.classify([0, 0, 225]);

// Now find the major one
...
var viewListCounter = 0;
var oldAns = null;
window.setInterval(function(){

  var newAns = getView();
  if(arraysEqual(newAns,oldAns)){
}
else{
  viewList.push(newAns);
  oldAns = newAns;
}
...
getClusters(50);
// Now set this DIV with the answers
...
function getViewICM(){
// Get the view 
act.RunCommandsValue("View()");

}
function setViewICM(inArray){
out_s = "set view {"
var out_arr = new Array()
for(i=0;i<view_set.length;i++){
out_arr.push(view_set[i].toFixed(5))
}
